...
func SliceRepeat(m dsl.Matcher) {
	...
	// `for i := range items` over a slice (ranging the slice, not a count)
	// matches and yields a non-compiling slices.Repeat(s, items), and
	// slices.Repeat's count parameter is exactly int, so a named int type or
	// int64 would not compile either. Type.Is("int") also covers an untyped
	// constant bound (`for range 5`), which go/types gives the default type int
	// in range context.
	m.Match(
